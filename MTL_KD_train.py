...
def train(epoch, lrc, model, teacher_model, dataloader, dict_dataloader_val, text_field):
    '''
    Optimization Stage 5: Fine-tune the independently trained task model with combined loss
    Takes in: Epoch no, learning rate, student model, teacher model, train dataloader, test dataloader and text_field.
    ...
    for epoch_count in range(epoch):
        ...
        if args.gsu_cbs:
            model.scene_graph.grnn1.gnn.apply_h_h_edge.get_new_kernels(epoch_count)
        model = model.to(device)
        ...
        for data in tqdm(dataloader):
            ...
            # forward propagation for student model
            ...
            with torch.no_grad():
                teacher_interaction, teacher_caption = teacher_model(img_loc, det_boxes, caps_gt, node_num, spatial_feat, word2vec, roi_labels)
        
            # KL-divergence loss between the teacher and student model
            kl_scene_graph_loss = F.kl_div(interaction, teacher_interaction)
            kl_caption_loss = F.kl_div(caption, teacher_caption)
            ...
            #Knowledge distillation, KL loss + CE loss
            loss = (0.35 * scene_graph_loss) + (0.35 * caption_loss) + (0.15 * kl_scene_graph_loss) + (0.15 * kl_caption_loss)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cp_decay_epoch',        type=int,       default=4) 
    ...
    parser.add_argument('--mtl_version',           type=str,       default='KL_UDA_SC_CBS')
    ...
    '''
    teacher_model = build_model(args, text_field, device)
    model = build_model(args, text_field, device, load_pretrain=False)

    '''================================= initial model evaluation ================================'''
    print('=========== techer Model ===========')
    eval_mtl(teacher_model, dict_dataloader_val, text_field)
    print('=========== child Model ============')
    eval_mtl(model, dict_dataloader_val, text_field)

    '''Algorithm 2, Optimization Stage 3: Train MTL model based on KL-divergence and CE loss'''
    print('Learning_rate: ', args.lr)
    train(args.epoch, args.lr, model, teacher_model, train_dataloader, dict_dataloader_val, text_field)
