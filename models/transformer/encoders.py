...
from models.transformer.gaussian_kernel import get_gaussian_filter_1D, get_laplaceOfGaussian_filter_1D
...
class MemoryAugmentedEncoder_CBS(nn.Module):
    def __init__(self, N, padding_idx, d_in=512, d_model=512, dropout=.1, std=1, cbs_filter='LOG', **kwargs): # For CBS: std=1, cbs_filter='LOG', 
        super(MemoryAugmentedEncoder_CBS, self).__init__()
        self.d_model = d_model
        self.dropout = dropout

        self.std = std
        self.cbs_filter = cbs_filter
    ...
    def get_new_kernels(self, epoch_count, kernel_sizex, kernel_sizey, decay_epoch, std_factor, cbs_filter):     
        if epoch_count % decay_epoch == 0 and epoch_count is not 0:
            self.std *= std_factor 
        if (cbs_filter == 'gau'): 
            self.kernel1 = get_gaussian_filter_1D(kernel_sizex=kernel_sizex, kernel_sizey=kernel_sizey, sigma=self.std, channels=6) # max_detections=6
        elif (cbs_filter == 'LOG'): 
            self.kernel1 = get_laplaceOfGaussian_filter_1D(kernel_size=kernel_sizex, sigma=self.std, channels=6)
